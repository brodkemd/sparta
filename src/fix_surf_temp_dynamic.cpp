...
FixSurfTempDynamic::FixSurfTempDynamic(SPARTA *sparta, int narg, char **arg) :
  Fix(sparta, narg, arg)
{
  if (narg != 8) error->all(FLERR,"Illegal fix surf/temp/dynamic command");
  ...
  if (igroup < 0) error->all(FLERR,"Fix surf/temp/dynamic group ID does not exist");
  ...
  if (strncmp(arg[4],"c_",2) == 0) {
    ...
    if (icompute < 0) error->all(FLERR,"Could not find fix surf/temp/dynamic compute ID");
    ...
    fqw = modify->fix[ifix];
    if (ifix < 0) error->all(FLERR,"Could not find fix surf/temp/dynamic fix ID");
  ...
  if (twall <= 0.0) error->all(FLERR,"Fix surf/temp/dynamic initial temp <= 0.0");
...
int FixSurfTempDynamic::setmask()
{
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
